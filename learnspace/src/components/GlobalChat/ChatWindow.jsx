...
import { X, Minus, Send, Phone, Video } from "lucide-react";
      ...
      >
        <div className="flex items-center gap-2 overflow-hidden">
          <div className="relative">
             <img src={chatData.avatar} alt="avatar" className="w-8 h-8 rounded-full object-cover shrink-0" />
             <div className="absolute bottom-0 right-0 w-2.5 h-2.5 bg-green-500 border border-white rounded-full"></div>
          </div>
          <div className="font-semibold text-sm text-gray-800 truncate">{chatData.teacherName}</div>
        </div>
        <button onClick={(e) => { e.stopPropagation(); handleClose(); }} className="p-1 hover:bg-gray-200 rounded-full text-gray-500">
          <X size={16} />
        </button>
      </div>
    );
  ...
  return (
    <div className="w-[330px] h-[450px] bg-white rounded-t-xl shadow-[0_-5px_25px_rgba(0,0,0,0.15)] border border-gray-200 border-b-0 flex flex-col overflow-hidden animate-in slide-in-from-bottom-2 duration-200">
      ...
      <div className="h-14 border-b border-gray-100 flex items-center justify-between px-3 bg-white shrink-0 shadow-sm z-10">
        <div className="flex items-center gap-2 cursor-pointer w-full overflow-hidden" onClick={handleMinimize}>
          <div className="relative shrink-0">
             <img src={chatData.avatar} alt="avatar" className="w-9 h-9 rounded-full object-cover" />
             <div className="absolute bottom-0 right-0 w-2.5 h-2.5 bg-green-500 border border-white rounded-full"></div>
          </div>
          <div className="flex flex-col min-w-0">
            <span className="font-bold text-[14px] text-gray-900 leading-tight truncate">{chatData.teacherName}</span>
            <span className="text-[11px] text-gray-500 leading-tight">Đang hoạt động</span>
          </div>
        </div>
        <div className="flex items-center gap-1 text-purple-600 shrink-0">
          <button className="p-1.5 hover:bg-gray-100 rounded-full transition-colors"><Phone size={18} /></button>
          <button className="p-1.5 hover:bg-gray-100 rounded-full transition-colors"><Video size={18} /></button>
          <button onClick={handleMinimize} className="p-1.5 hover:bg-gray-100 rounded-full transition-colors text-gray-400 hover:text-gray-600"><Minus size={18} /></button>
          <button onClick={handleClose} className="p-1.5 hover:bg-gray-100 rounded-full transition-colors text-gray-400 hover:text-gray-600"><X size={18} /></button>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-3 flex flex-col gap-3 bg-white scrollbar-hide">
        <div className="flex justify-center my-4">
          <div className="flex flex-col items-center gap-1">
             <img src={chatData.avatar} className="w-16 h-16 rounded-full object-cover shadow-sm" alt="avatar" />
             <div className="font-bold text-gray-900 mt-1">{chatData.teacherName}</div>
        ...
        <div className="flex gap-2">
          <img src={chatData.avatar} className="w-7 h-7 rounded-full shrink-0 object-cover mt-auto" alt="avatar" />
